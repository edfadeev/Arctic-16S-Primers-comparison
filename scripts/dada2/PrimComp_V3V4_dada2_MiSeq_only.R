...
fnFs_mi1 <- sort(file.path("Clipped",paste(c(1:37), "_clip_R1.fastq", sep = "")))
fnRs_mi1 <- sort(file.path("Clipped",paste(c(1:37), "_clip_R2.fastq", sep = "")))
filtFs_mi1 <- sort(file.path("Filtered",paste(c(1:37), "_F_filt.fastq.gz", sep = "")))
filtRs_mi1 <- sort(file.path("Filtered",paste(c(1:37), "_R_filt.fastq.gz", sep = "")))
#Filter and trim
...
seqtab1<- makeSequenceTable(mergers_mi1)
...
# get summary tables 
sample.order <- names(c(dadaFs_mi1))

getN <- function(x) sum(getUniques(x))
track <- cbind(rbind(out_mi1), 
               sapply(c(dadaFs_mi1), getN),
               sapply(c(mergers_mi1), getN),
