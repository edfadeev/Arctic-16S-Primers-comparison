...
# list files
path <- "/projectnb/npsegre/efadeev_playground/Primer_analysis/V4V5"
...
#Filter and trim
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=10, truncLen=c(260,170),
...
# write output
write.table(seqtab.nochim.print, file.path(path,"V4V5_seqtab_nochim.txt"), quote = F, sep = "\t")
write.table(taxa, file.path(path,"V4V5_taxonomy_table.txt"), sep = "\t", quote = F)

save.image(file.path(path,"dada2_V4V5.Rdata"))
